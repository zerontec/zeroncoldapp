...
const UserSystem = () => {
  ...
  const validateForm = () => {
    ...
    setIsFormValid(
      name.trim() !== '' && username.trim() !== '' && email.trim() !== '' && password.trim() !== '' && roles.length > 0
    );
  };
  ...
  function validate(formInfo) {
    ...
    formInfo.username ? (errors.username = '') : (errors.username = 'Ingrese una Descripcion');

    formInfo.email ? (errors.email = '') : (errors.email = 'Ingrese un email');

    formInfo.password ? (errors.password = '') : (errors.password = 'Ingrese un password');

    formInfo.roles.length > 0 ? (errors.roles = '') : (errors.roles = 'Seleccione al menos un rol');
  ...
  const handleSubmitEdit = (e) => {
    if (
      ...
      const data = {
        ...selectedUserEdit,
        id: selectedUserId,
        roles: [selectedUserEdit.role],
      };
      dispatch(updateUser(selectedUserId, data));
      Swal.fire('¨Usuaro Editado con Exito  !', 'You clicked the button!', 'success');
